...
export function pathValue(env, platform = process.platform) {
    return env[pathKey(env, platform)] ?? '';
}
function assignPath(env, value, platform = process.platform) {
    const key = pathKey(env, platform);
    for (const existing of Object.keys(env)) {
        if (existing.toUpperCase() === 'PATH' && existing !== key)
            delete env[existing];
    }
    env[key] = value;
}
