...
export class PnpmProgressTracker {
    ...
    push(chunk, operation) {
        ...
        if (lastSample !== undefined && now - lastSample.at >= 5000)
    ...
    consume(line, operation) {
        ...
        if (event.name === 'pnpm:stage') {
            if (event.stage === 'resolution_started')
                this.stage(operation, 'resolving');
            else if (event.stage === 'resolution_done')
                this.stage(operation, 'downloading');
            else if (event.stage === 'importing_started' || event.stage === 'importing_done')
        ...
        if (event.name === 'pnpm:progress') {
            if (event.status === 'resolved' && operation.pnpmStage === undefined)
                this.stage(operation, 'resolving');
            else if (event.status === 'fetched' || event.status === 'found_in_store')
                this.stage(operation, 'downloading');
            else if (event.status === 'imported')
                this.stage(operation, 'linking');
        }
        if (event.name === 'pnpm:fetching-progress')
            this.stage(operation, 'downloading');
    ...
    publish(operation) {
        ...
        if (fetches.every(item => item.size !== undefined))
            operation.totalBytes = fetches.reduce((sum, item) => sum + item.size, 0);
        else
