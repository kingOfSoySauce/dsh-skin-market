...
export function reviewedNpmSourceError(skin) {
    const npm = skin.install.npm;
    if (npm === undefined)
        return '尚无已核验的 npm 安装来源';
    if (npm.name !== skin.package)
        return 'npm 包名与目录皮肤不一致';
    if (npm.version !== skin.install.version)
        return 'npm 版本与目录固定版本不一致';
    if (!/^[0-9a-f]{40}$/i.test(npm.gitHead) || npm.gitHead.toLowerCase() !== skin.install.commit.toLowerCase())
        return 'npm gitHead 与目录固定 commit 不一致';
    const repository = repositoryIdentity(npm.repository);
    if (repository === null || repository !== repositoryIdentity(skin.repo))
        return 'npm repository 与目录仓库不一致';
    if (!/^sha(?:1|256|384|512)-[A-Za-z0-9+/]+={0,2}$/.test(npm.integrity))
        return 'npm integrity 缺失或格式无效';
    return null;
}
function isNpmDependencySpec(spec, packageName) {
    let version = spec;
