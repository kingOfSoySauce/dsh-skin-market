...
function isConstrainedHost(platform = process.platform, env = process.env, memoryBytes = totalmem()) {
    if (platform === 'android')
        return true;
    if ((env.ANDROID_ROOT ?? '').trim() !== '' || (env.ANDROID_DATA ?? '').trim() !== '')
        return true;
    return memoryBytes > 0 && memoryBytes < LOW_MEMORY_BYTES;
}
/** Isolated temp-dir prefetches may overlap; live profile pnpm stays serial. */
export function maxParallelDownloads(platform = process.platform, env = process.env, memoryBytes = totalmem()) {
...
function removePrefetchDirectory(directory) {
    try {
...
export class SkinLifecycle {
    ...
    downloadAvailable = maxParallelDownloads();
    downloadWaiters = [];
    ...
    currentOperation() {
        ...
        const rank = (operation) => {
            if (operation.phase === 'installing' || operation.phase === 'validating' || operation.phase === 'activating' || operation.phase === 'cancelling')
                return 0;
            if (operation.phase === 'downloading' || operation.phase === 'resolving')
                return 1;
            return 2;
        };
    ...
    async withProfileLock(operation, fn) {
        if (this.profileLockOwner === operation.id)
            return await fn();
        ...
        try {
            ...
            if (this.profileLockOwner === operation.id)
                this.profileLockOwner = null;
            release();
        }
    }
    async withDownloadSlot(operation, fn) {
        this.checkDeadline(operation);
    ...
    async installPackage(skin, operation, state, target, prefetched) {
        if (this.hostKind === 'desktop') {
    ...
    async installIntoProfile(operation, skin, prefetched) {
        if (this.hostKind !== 'desktop') {
    ...
    async activateLocked(operation) {
        const skin = this.skin(operation.skinId);
    ...
    async deactivateLocked(operation) {
        const skin = this.skin(operation.skinId);
        this.update(operation, 'activating');
        const previous = readMarketState(this.options.profileDir);
    ...
    async uninstallLocked(operation) {
        const skin = this.skin(operation.skinId);
        if (readDependencies(this.options.profileDir)[skin.package] === undefined)
