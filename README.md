...
具有已核验 npm 来源的目录可以优先通过 npm 精确版本安装；没有合格 npm 来源时，继续使用原来的 GitHub 固定 commit。npm 来源须与目录的包名、版本、仓库、完整 commit 一致，并通过安装包完整性与已构建客户端入口检查。本次代码更新兼容已有的 GitHub 安装；公共目录的 npm 来源回填将在兼容版本发布后启用，避免旧版市场将已有 GitHub 安装误判为损坏。
